...
export class ChatService {
	...
	async createChat(jwtPayloadDto: JwtPayloadDto, createChatDto: CreateChatDto) {
		...
		if (createChatDto.password && createChatDto.status !== ChatRoomStatus.PROTECTED)
	...
	async updateChat(jwtPayloadDto: JwtPayloadDto, chatIndex: number, updateChatDto: UpdateChatDto) {
		...
		if (updateChatDto.status !== ChatRoomStatus.PROTECTED && updateChatDto.password)
	...
	async registerAdmin(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		...
		if (chat.adminUsers.find((adminUser) => {
	...
	async unRegisterAdmin(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		const user = await this.userRepository.findOneOrFail({
			where: { username: username },
		});
		const chat = await this.chatRoomRepository.findOneOrFail({
	...
	async registerMuteUser(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		const user = await this.userRepository.findOneOrFail({
			where: { username: username },
		});
		const chat = await this.chatRoomRepository.findOneOrFail({
		...
		// if (this.existUserInChat(user.index, chat) === false)
		// 	throw new BadRequestException('Do not join user this chat');
		if (chat.mutedUsers.find((mutedUser) => {
	...
	async unRegisterMuteUser(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		const user = await this.userRepository.findOneOrFail({
			where: { username: username },
		});
		const chat = await this.chatRoomRepository.findOneOrFail({
	...
	async registerBanUser(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		const user = await this.userRepository.findOneOrFail({
			where: { username: username },
		});
		const chat = await this.chatRoomRepository.findOneOrFail({
		...
		// if (this.existUserInChat(user.index, chat) === false)
		// 	throw new BadRequestException('Do not join user this chat');
		if (chat.bannedUsers.find((bannedUser) => {
	...
	async unRegisterBanUser(jwtPayloadDto: JwtPayloadDto, chatIndex: number, username: string) {
		const user = await this.userRepository.findOneOrFail({
			where: { username: username },
		});
		const chat = await this.chatRoomRepository.findOneOrFail({
	...
	existUserInChat(userIndex: number, chatRoom: ChatRoom): Boolean {
		for (let index = 0; index < chatRoom.joinUsers.length; index++) {
			if (chatRoom.joinUsers[index].index === userIndex)
				return true;
		}
		return false;
	}
